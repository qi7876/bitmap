...
bool parseTagQuery(const std::string& input, bitmap_index::core::StringTagSet& tags, bitmap_index::core::BitmapOperation& op) {
    tags.clear();
...
int main(int argc, char* argv[]) {
    // --- Configuration ---
    // TODO: Use command-line arguments or a config file
    std::string data_file_path = "data.csv"; // Default data file name
    std::string status_file_path = "index_status.txt"; // Default status file
    char delimiter = '|';
    ...
    if (!manager.loadIncremental()) {
        std::cerr << "Error loading index data. Exiting." << std::endl;
        return 1;
    }
    ...
    std::string line;
    while (true) {
        std::cout << "\nEnter query type ('tagsfor <doc_id>', 'query <tag1> <tag2>... <OPERATION>', or 'quit'):" << std::endl;
        std::cout << "> ";
