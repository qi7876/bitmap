...
#include <filesystem> // Ensure filesystem is included
...
namespace bitmap_index::core
{
    ...
    void IndexManager::processParsedLine(StringId&& id_str, StringTagSet&& tags_str)
    {
        ...
        {
            // Iterate through original list including duplicates if needed by inverted logic
    ...
    bool IndexManager::loadIncremental(bool optimize_after_load)
    {
        ...
        {
            std::cerr << "Warning: Could not determine stream position after parsing. Using file size as offset." <<
                std::endl;
    ...
    StringIdList IndexManager::queryTags(const StringTagSet& tags, BitmapOperation op)
    {
        ...
        for (auto it = result_bitmap.begin(); it != result_bitmap.end(); ++it)
        {
            ...
            if (!str_id.empty())
            {
                // Should always be found if in bitmap
                result_ids.push_back(str_id);
            }
            else
            {
                std::cerr << "Warning: Found DocId " << doc_id <<
                    " in bitmap result, but no corresponding StringId found in mapping." << std::endl;
            }
        }
    ...
    StringTagSet IndexManager::getTagsForDocument(const StringId& doc_id_str)
    {
        ...
        for (TagId tag_id : tag_ids)
        {
            ...
            if (!tag_str.empty())
            {
                // Should always be found if in forward index
    ...
    bool IndexManager::saveIndex(const std::filesystem::path& directory) const
    {
        // std::shared_lock lock(manager_mutex_); // Acquire read lock for saving consistent state
        ...
        try
        {
            ...
            if (ec)
            {
                std::cerr << "Error creating directory '" << directory.string() << "': " << ec.message() << std::endl;
                return false;
            }

            // Define file paths
            fs::path mapping_file = directory / "mapping.dat";
            fs::path forward_file = directory / "forward.dat";
            fs::path inverted_file = directory / "inverted.dat";

            // Save Mapping
            {
                std::ofstream ofs(mapping_file, std::ios::binary | std::ios::trunc);
                if (!ofs)
                {
                    std::cerr << "Error opening " << mapping_file << " for writing." << std::endl;
                    return false;
                }
                // ---> REMOVE PLACEHOLDER, CALL ACTUAL SAVE <---
                if (!mapping_.save(ofs))
                {
                    std::cerr << "Error saving Mapping." << std::endl;
                    return false;
                }
                std::cout << "Mapping saved successfully." << std::endl;
            }

            // Save Forward Index
            {
                std::ofstream ofs(forward_file, std::ios::binary | std::ios::trunc);
                if (!ofs)
                {
                    std::cerr << "Error opening " << forward_file << " for writing." << std::endl;
                    return false;
                }
                // ---> REMOVE PLACEHOLDER, CALL ACTUAL SAVE <---
                if (!forward_index_.save(ofs))
                {
                    std::cerr << "Error saving ForwardIndex." << std::endl;
                    return false;
                }
                std::cout << "ForwardIndex saved successfully." << std::endl;
            }

            // Save Inverted Index
            {
                std::ofstream ofs(inverted_file, std::ios::binary | std::ios::trunc);
                if (!ofs)
                {
                    std::cerr << "Error opening " << inverted_file << " for writing." << std::endl;
                    return false;
                }
                if (!inverted_index_.save(ofs))
                {
                    std::cerr << "Error saving InvertedIndex." << std::endl;
                    return false;
                }
                std::cout << "InvertedIndex saved successfully." << std::endl;
            }
        }
        ...
        std::cout << "Index save process completed." << std::endl;
        return true;
    }


    bool IndexManager::loadIndex(const std::filesystem::path& directory)
    {
        std::unique_lock lock(manager_mutex_);

        // ---> FIX: Declare file paths *before* clear() and try block <---
        fs::path mapping_file = directory / "mapping.dat";
        fs::path forward_file = directory / "forward.dat";
        fs::path inverted_file = directory / "inverted.dat";

        mapping_.clear(); // Now Mapping::clear() exists (added above)
        forward_index_.clear();
        inverted_index_.clear(); // Now InvertedIndex::clear() exists (added above)

        try
        {
            // Check if all files exist
            // ---> FIX: Use variables declared above <---
            if (!fs::exists(mapping_file) || !fs::exists(forward_file) || !fs::exists(inverted_file))
            {
                std::cerr << "Info: Index files not found in '" << directory.string() << "'. Starting with empty index."
                    << std::endl;
                return false;
            }
            ...
            // Load Mapping
            {
                ...
                // ---> REMOVE PLACEHOLDER, CALL ACTUAL LOAD <---
                if (!mapping_.load(ifs))
                {
                    std::cerr << "Error loading Mapping." << std::endl;
                    clearIndicesOnError(); // Helper or manual clear
                    return false;
                }
                std::cout << "Mapping loaded successfully." << std::endl;
            }

            // Load Forward Index
            {
                std::ifstream ifs(forward_file, std::ios::binary);
                if (!ifs)
                {
                    /*...*/
                    return false;
                ...
                // ---> REMOVE PLACEHOLDER, CALL ACTUAL LOAD <---
                if (!forward_index_.load(ifs))
                {
                    std::cerr << "Error loading ForwardIndex." << std::endl;
                    clearIndicesOnError(); // Helper or manual clear
                    return false;
                }
                std::cout << "ForwardIndex loaded successfully." << std::endl;
            }

            // Load Inverted Index
            {
                std::ifstream ifs(inverted_file, std::ios::binary);
                if (!ifs)
                {
                    /*...*/
                    return false;
                }
                if (!inverted_index_.load(ifs))
                {
                    std::cerr << "Error loading InvertedIndex." << std::endl;
                    clearIndicesOnError(); // Helper or manual clear
                    return false;
                }
                std::cout << "InvertedIndex loaded successfully." << std::endl;
            }
        }
        catch (const std::exception& e)
        {
            std::cerr << "Error during loadIndex: " << e.what() << std::endl;
            // Already cleared at the top, just return false
            return false;
        }
        std::cout << "Index load process completed (Placeholders for Mapping/Forward)." << std::endl;

        last_processed_offset_ = readLastOffset();
        std::cout << "Offset after loadIndex set to: " << last_processed_offset_ << std::endl;

        return true;
    }

    // Helper to avoid repetition (optional)
    void IndexManager::clearIndicesOnError()
    {
        mapping_.clear();
        forward_index_.clear();
